...
<template>
  <li class="relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12" :class="colorClass" :aria-current="current ? 'step' : undefined">
    ...
    <div class="md:min-h-[4rem] md:order-1">
      <div v-if="side === 'left'" class="md:max-w-prose md:ml-auto" :class="{ 'blur-sm': blurred }">
        ...
        <div class="mt-3 md:flex md:justify-end">
          <NuxtLink
            v-if="event.href"
            :to="event.href"
            class="inline-flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm no-underline hover:bg-[var(--color-surface)]/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--dot)]"
            :aria-label="t('timeline.more_aria', { title: event.title })"
            @click="onClick"
          >
            {{ t('timeline.more') }}
          </NuxtLink>
    ...
    <div class="md:min-h-[4rem] md:order-2">
      <div v-if="side === 'right'" class="md:max-w-prose" :class="{ 'blur-sm': blurred }">
