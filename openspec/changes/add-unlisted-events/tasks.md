...
- [ ] 4.1 `useEventDetail` entscheidet 404 über `isEventReachableAt`; `EventDetail.phase` wird zu `EventPhase`; ein Composable-Test (nuxt-Environment, `mockNuxtImport` um das Original gewickelt) belegt: nicht gelistet + `hidden` → Detail mit Phase `hidden`; gelistet + `hidden` → 404; unbekannter Slug → 404
- [ ] 4.2 `EventPhaseChip` und `EventTestingBlock` akzeptieren `hidden` (Label „Vorschau“/„Preview“ neutral; Feedback-Link wie bei `announced`); `events.phase.hidden` und `events.phase_hint.hidden` in `de.json` und `en.json` ergänzen; verifiziert durch Komponententests in `tests/events/event-components.spec.ts` und die i18n-Schlüsselprüfung der Suite
