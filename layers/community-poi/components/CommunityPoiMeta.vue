...
const builderLinkClass
  = 'rounded-extra-small underline decoration-dotted underline-offset-2 hover:text-primary focus-ring'
</script>

<template>
  <dl class="grid gap-x-6 gap-y-3 text-body-medium sm:grid-cols-2">
    <div v-if="poi.location">
      <dt class="text-label-large text-on-surface-variant">
        {{ t('community_poi.meta.location') }}
      </dt>
      <dd class="text-on-surface">{{ poi.location }}</dd>
    </div>
    <div v-if="coordsLabel">
      ...
      <dd class="text-on-surface">
        ...
        <span v-if="dimensionLabel" class="ml-1 text-on-surface-variant">
          ({{ dimensionLabel }})
        </span>
      ...
      <dd class="text-on-surface">
        <ul class="flex flex-wrap gap-x-2 gap-y-1">
          <li v-for="(b, i) in builders" :key="b.name + i" class="inline">
            ...
            >
              {{ b.name }}
              <span v-if="b.mcName" class="ml-1 text-on-surface-variant">({{ b.mcName }})</span>
            </a>
            <span v-else>
              {{ b.name }}
              <span v-if="b.mcName" class="ml-1 text-on-surface-variant">({{ b.mcName }})</span>
