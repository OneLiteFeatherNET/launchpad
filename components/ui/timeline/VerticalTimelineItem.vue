...
const props = withDefaults(defineProps<{
  event: TimelineEvent
  index?: number
  // Position erzwingen (sonst via Parent alternierend)
  position?: TimelineSide
  // Farbschema für den Marker
  colorVariant?: TimelineColorVariant
  // Markiert das aktuelle/aktive Event
  current?: boolean
}>(), {
  colorVariant: 'brand',
  current: false,
...
const formattedDate = computed(() => {
  ...
  // Fallback: ISO‑String, ansonsten D.M.YYYY
...
<template>
  <li class="relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12" :class="colorClass" :aria-current="current ? 'step' : undefined">
    <!-- Linke Spalte (für alternierendes Layout) -->
    <div class="md:min-h-[4rem]" :class="[side === 'left' ? 'md:order-2' : 'md:order-1']">
      <div v-if="side === 'right'" class="md:ml-auto md:max-w-prose">
        <header class="flex items-start gap-3">
          <div class="mt-1 size-3 shrink-0 rounded-full bg-[var(--dot)] ring-4 ring-[var(--color-surface)] dark:ring-neutral-900" />
          <div>
    ...
    <!-- Rechte Spalte -->
    <div class="md:min-h-[4rem]" :class="[side === 'left' ? 'md:order-1' : 'md:order-2']">
      <div v-if="side === 'left'" class="md:max-w-prose">
        <header class="flex items-start gap-3">
          <div class="mt-1 size-3 shrink-0 rounded-full bg-[var(--dot)] ring-4 ring-[var(--color-surface)] dark:ring-neutral-900" />
          <div>
            <div class="text-sm text-[var(--color-muted)]"> <slot name="date">{{ formattedDate }}</slot> </div>
    ...
    <!-- Linie wird durch den Parent gerendert -->
  </li>
</template>
