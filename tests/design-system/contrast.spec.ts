...
import { schemeColors, seasonCss, seasonIds, themeCss, type SchemeColor } from '../helpers/theme'
...
describe('seasonal colour roles', () => {
  // A season is generated by the same maths from other seeds, and it is held
  // to the same pairs: a costume that fails AA fails the audience it dresses.
  it.each(seasonIds())('%s meets every contrast minimum in both schemes', (id) => {
    const failures = contrastFailures(schemeColors(seasonCss(id) ?? ''))
    expect(failures.map((failure) => `${id}: ${failure}`)).toEqual([])
  })

  it('name the season, pair, scheme and ratio of a failing value', () => {
    const tampered = seasonCss('halloween')?.replace(
      /--color-on-secondary: light-dark\(#[0-9a-f]{6}/,
      '--color-on-secondary: light-dark(#ff7518',
    ) ?? ''
    const failures = contrastFailures(schemeColors(tampered)).map((failure) => `halloween: ${failure}`)
    expect(failures).toHaveLength(1)
    expect(failures[0])
      .toMatch(/^halloween: on-secondary on secondary \(light\): \d\.\d{2} < 4\.5$/)
  })
})
